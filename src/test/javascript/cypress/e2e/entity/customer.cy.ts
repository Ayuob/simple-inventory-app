...
describe('Customer e2e test', () => {
  ...
  const customerSample = { customerId: 'bluetooth client-server' };
  ...
  describe('new Customer page', () => {
    ...
    it('should create an instance of Customer', () => {
      cy.get(`[data-cy="customerId"]`).type('virtual').should('have.value', 'virtual');

      cy.get(`[data-cy="company"]`).type('teal').should('have.value', 'teal');

      cy.get(`[data-cy="address"]`).type('Swiss azure').should('have.value', 'Swiss azure');

      cy.get(`[data-cy="city"]`).type('North Letitia').should('have.value', 'North Letitia');

      cy.get(`[data-cy="state"]`).type('cross-platform').should('have.value', 'cross-platform');
